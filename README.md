...
> `usePush` sequer era chamado por algum componente, logo não havia botão na
> interface. Ganhou par VAPID, a tabela `push_subscriptions`
> (`db/001_push_subscriptions.sql`, o primeiro DDL versionado deste repo), as
> rotas `/api/v1/push/*` e o `NotificationsCard.vue` na Home.
